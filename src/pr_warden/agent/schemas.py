...
class DoneInput(ToolInput):
    ...
    verdict headline now comes from the model too (`verdict_level` + `verdict`),
    not a deterministic rule — the only exception is a hard HIGH-severity check
    (a leaked secret / critical-path touch), which the app floors to 🔴 so the
    model can never bury it.
    """
